...
class Game:
    ...
    def game_over(self):
        title = self.font.render('Игра окончена', True, WHITE)
        title_rect = title.get_rect(center=(WIN_WIDTH/2,WIN_HEIGHT/2))
        restart_button = Button(WIN_WIDTH/2-100,WIN_HEIGHT/2+70,200,50,WHITE,BLACK,'Отомстить',32)
        ...
        while self.running:
            ...
            if restart_button.is_pressed(mouse_pos,mouse_pressed):
                self.new()
                self.main()
            self.screen.blit(self.go_bacgraund,(0,0))
